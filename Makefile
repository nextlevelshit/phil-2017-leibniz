...
pdf:
	pandoc metadata.yaml --bibliography library.bib --template=template.tex --smart --filter pandoc-citeproc --variable documentclass=templates/sig-alternate --csl=templates/acm-sig-proceedings.csl --highlight-style=haddock -f markdown -t latex -o out/paper.pdf paper.md
html:
